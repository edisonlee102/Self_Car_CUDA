...
class Polyfitter:
	...
	leftx = [int(i*xm_per_pix) for i in left_fit(ploty)]
	rightx = [int(i*xm_per_pix) for i in right_fit(ploty)]
    
	#Identify new coefficients in metres
	#print ploty, '\n', ym_per_pix, '\n', leftx, '\n', xm_per_pix
	left_fit_cr = np.polyfit(list(ploty*ym_per_pix), leftx, 2)
	right_fit_cr = np.polyfit(list(ploty*ym_per_pix), rightx, 2)
	...
	left_lane_bottom = int(left_fit(y_eval))
	right_lane_bottom = int(right_fit(y_eval))
