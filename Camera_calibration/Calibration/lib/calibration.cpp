...
void CameraCalibrator::addChessboardPoints(){  
    ...
    for(int i=0; i<m_filenames.size(); i++){
        ...
        bool found = findChessboardCorners(image, m_borderSize, srcCandidateCorners);
	if(found){
	...
	cv::drawChessboardCorners(image, m_borderSize, srcCandidateCorners, found);
	cv::imshow("Corners on Chessboard", image);
        cv::waitKey(500); 
    }
    cout << "Find avild picture number : " << successes << endl;
...
void CameraCalibrator::calibrate(Size imageSize){ 
...
cv::Mat CameraCalibrator::QciRemap(const cv::Mat &image){
    Mat undistorted;
    remap(image, undistorted, map1, map2, INTER_LINEAR);
    // It's auto judge free memory or not by system.
    return undistorted;
}
